...
```bash
npm @andrew-nazin/single-flight
```
...
const singleFlight = new SingleFlight();
let calls = 0;
const operation = async () => { calls += 1; await new Promise((resolve) => setTimeout(resolve, 100)); return "result"; };
const [first, second, third] = await Promise.all;
console.log(first); console.log(second); console.log(third); console.log(calls);
singleFlight.dispose();
```
...
try { await singleFlight.exec("resource", async () => { throw new Error("Temporary failure"); }); } catch (error) { console.error(error); }
try { await singleFlight.exec("resource", async () => { return "this will not run during cooldown"; }); } catch (error) { console.error(error); }
singleFlight.dispose();
```
...
interface SingleFlightOptions<TValue, TError extends Error = Error> { cooldownMs?: number; normalizeError?: (error: unknown) => TError; shouldRetry?: (error: TError) => boolean; onSuccess?: (value: TValue) => void | Promise ; onError?: (error: TError) => void | Promise; }
interface SingleFlightState<TValue, TError extends Error = Error> { inFlight: Promise| null; lastError: TError | null; errorCacheExpiresAt: number; }
```
